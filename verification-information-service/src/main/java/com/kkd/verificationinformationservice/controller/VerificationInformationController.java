...
public class VerificationInformationController{
	...
	@GetMapping("/generateotp/{aadharNo}")
	public ResponseEntity<String> generateOtp(@PathVariable String aadharNo) {
		System.out.println("vbkasjdfhasdukfhisdfhsdifsadfsdjklf"+aadharNo);
		String mobileNo=proxy.addAadhar(aadharNo);
		System.out.println("asdkjfbhsdjkfhasjkdfhasjkldghsadkjghsadjkg"+mobileNo);
	...
	//Fallback method for the above method.
		public ResponseEntity<String> generateOtpFallback(@PathVariable String mobileNo) {
			messageSenderService.produceMsg("Error in fetching the required data.");
	...
	public ResponseEntity<List<OTPBean>> getAllActiveOtp(){
		messageSenderService.produceMsg("Fetching all the OTPs and the mobile numbers that they are corresponding to.");
	...
	public ResponseEntity<Boolean> verifyOtp(@RequestBody OTPBean dataInBody){
		messageSenderService.produceMsg("Verifying the OTP.");
	...
	public ResponseEntity<Boolean> verifyOtpFallback(@RequestBody OTPBean dataInBody) {
		messageSenderService.produceMsg("Error in verifying the OTP.");
		Boolean bool = false; 
		return ResponseEntity.status(HttpStatus.NOT_FOUND).body(bool);
	}
	...
	public ResponseEntity<Integer> fetchOtp(@PathVariable String mobileNo){
		...
		messageSenderService.produceMsg("Got the OTP");
		OTPBean Otp;
		int gotOtp = 0;
